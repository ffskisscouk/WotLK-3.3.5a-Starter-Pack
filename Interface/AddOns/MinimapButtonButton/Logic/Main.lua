...
local function collectButton (button)
  ...
  if (button.SetIgnoreParentScale ~= nil) then
...
local function collectLibDBIconButtons ()
  ...
  -- Modern LibDBIcon; older 3.3.5-era copies only expose lib.objects (no GetButtonList).
  if (LibDBIcon.GetButtonList ~= nil and LibDBIcon.GetMinimapButton ~= nil) then
  ...
  elseif (LibDBIcon.objects ~= nil) then
...
local function initMainButton ()
  ...
  -- Frames ignore mouse until enabled (required on 3.3.5a; retail BackdropTemplate often masked this).
  mainButton:EnableMouse(true);
  -- Lets StartMoving() work from OnMouseDown on older clients (same pattern as LibDBIcon minimap buttons).
  mainButton:RegisterForDrag(LEFTBUTTON, MIDDLEBUTTON, 'RightButton');
  mainButton:SetSize(Constants.MAINBUTTON_DEFAULT_SIZE, Constants.MAINBUTTON_DEFAULT_SIZE);
...
local function hookLibDBIconButtons ()
  ...
  if (LibDBIcon.RegisterCallback ~= nil) then
